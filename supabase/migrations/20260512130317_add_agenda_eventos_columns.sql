-- Adiciona as colunas necessárias na tabela agenda_eventos para corrigir o erro "could not find the 'cliente_id' column"
...
ALTER TABLE public.agenda_eventos ADD COLUMN IF NOT EXISTS lead_id UUID REFERENCES public.leads(id) ON DELETE SET NULL;
ALTER TABLE public.agenda_eventos ADD COLUMN IF NOT EXISTS criado_por TEXT;

-- Atualiza a função create_demand_with_event para garantir a inserção correta do cliente_id na agenda
...
-- Força a atualização do cache da API do Supabase (PostgREST) para as novas colunas
