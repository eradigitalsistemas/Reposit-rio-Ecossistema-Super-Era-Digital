...
const timeEntrySchema = z.object({
  ...
  time: z.string().regex(/^\d{2}:\d{2}(:\d{2})?$/, 'Formato de hora inválido (HH:mm ou HH:mm:ss)').optional(),
  break_duration: z.string().regex(/^\d{2}:\d{2}(:\d{2})?$/, 'Duração do intervalo inválida').optional()
})
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!parsed.success) {
        return new Response(JSON.stringify({ error: 'Dados inválidos', details: parsed.error.errors }), {
          status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        })
      }
      ...
      const { data: emp, error: empErr } = await supabase.from('employees').select('id').eq('id', employee_id).single();
      if (empErr || !emp) {
      ...
      const { data: record } = await supabase.from('time_entries')
        .select('*')
        .eq('employee_id', employee_id)
        .eq('date', date)
        .maybeSingle();
      ...
      if (action === 'entry') {
        if (record && record.entry_time) {
          return new Response(JSON.stringify({ error: 'Já existe entrada registrada hoje' }), { status: 400, headers })
        }
        ...
        if (record) {
           const { data, error } = await supabase.from('time_entries').update({ entry_time: time, delay }).eq('id', record.id).select().single();
           if (error) throw error;
           return new Response(JSON.stringify({ data, success: true }), { status: 201, headers });
        } else {
           const { data, error } = await supabase.from('time_entries').insert({
             employee_id, date, entry_time: time, delay
           }).select().single();
           if (error) throw error;
           return new Response(JSON.stringify({ data, success: true }), { status: 201, headers });
        }
      ...
      if (action === 'exit') {
        if (!record || !record.entry_time) {
          return new Response(JSON.stringify({ error: 'Registre entrada antes de saída' }), { status: 400, headers })
        }
        if (record.exit_time) {
          return new Response(JSON.stringify({ error: 'Já existe saída registrada hoje' }), { status: 400, headers })
        }
        ...
        if (exitDecimal < entryDecimal) {
            return new Response(JSON.stringify({ error: 'Horário de saída não pode ser menor que entrada' }), { status: 400, headers })
        }
        ...
        if (hours_worked < 0) {
            return new Response(JSON.stringify({ error: 'Horário fora do expediente ou intervalo inválido' }), { status: 400, headers })
        }
      ...
      if (action === 'break') {
         if (!record || !record.entry_time) {
          return new Response(JSON.stringify({ error: 'Registre entrada antes do intervalo' }), { status: 400, headers })
        }
        ...
        const { data, error } = await supabase.from('time_entries').update(updateData).eq('id', record.id).select().single();
        if (error) throw error;
        return new Response(JSON.stringify({ data, success: true }), { status: 201, headers });
      }
    }
