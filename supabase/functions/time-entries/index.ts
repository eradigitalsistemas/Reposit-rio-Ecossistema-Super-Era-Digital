...
const timeEntrySchema = z.object({
  ...
  entry_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Formato de data inválido (YYYY-MM-DD)').refine(val => {
  ...
  entry_type: z.enum(['entrada', 'intervalo_saida', 'intervalo_entrada', 'saida'], {
    errorMap: () => ({ message: "entry_type deve ser um dos: entrada, intervalo_saida, intervalo_entrada, saida" })
  }),
  timestamp: z.string().datetime({ offset: true }).refine(val => val.endsWith('-03:00'), 'O timestamp deve estar no fuso horário UTC-03:00'),
  notes: z.string().max(500).optional()
})

const updateSchema = z.object({
  timestamp: z.string().datetime({ offset: true }).refine(val => val.endsWith('-03:00'), 'O timestamp deve estar no fuso horário UTC-03:00').optional(),
  notes: z.string().max(500).optional()
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (functionIndex === -1) {
       return new Response(JSON.stringify({ error: 'NOT_FOUND', message: 'Endpoint não encontrado' }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (req.method === 'GET') {
      ...
      if (params.length === 3) {
        ...
        function detectAnomalies(entries: any[]): any[] {
          ...
          if (sorted[0]?.entry_type !== 'entrada') {
            anomalies.push({ type: 'EXIT_WITHOUT_ENTRY', message: 'Registro iniciado sem entrada', severity: 'high' });
          }
          const intOut = sorted.find(e => e.entry_type === 'intervalo_saida');
          const intIn = sorted.find(e => e.entry_type === 'intervalo_entrada');
          if (intOut && intIn) {
            const intMins = Math.floor((new Date(intIn.timestamp).getTime() - new Date(intOut.timestamp).getTime()) / 60000);
            if (intMins < 30) {
              anomalies.push({ type: 'SHORT_BREAK', message: `Intervalo curto: ${intMins}min`, severity: 'medium' });
            }
          }
          const entrada = sorted.find(e => e.entry_type === 'entrada');
          const saida = sorted.find(e => e.entry_type === 'saida');
          if (entrada && saida) {
             const totalMins = Math.floor((new Date(saida.timestamp).getTime() - new Date(entrada.timestamp).getTime()) / 60000);
             if (totalMins > 12 * 60) {
                anomalies.push({ type: 'EXCESSIVE_HOURS', message: `Dia longo: ${Math.floor(totalMins / 60)}h ${totalMins % 60}min`, severity: 'medium' });
             }
          }
          return anomalies;
        }
        ...
        for (const [date, records] of Object.entries(grouped)) {
           totals.days_worked++;
           let dayHours = 0;
           let entry = null, intOut = null, intIn = null, out = null;
      ...
      if (params.length === 1) {
         ...
         if (error || !data) {
           return new Response(JSON.stringify({ error: 'NOT_FOUND', message: 'Registro de ponto não encontrado' }), {
    ...
    if (req.method === 'POST') {
      ...
      if (error) {
         ...
         if (msg.includes('DUPLICATE_ENTRY:')) {
           return new Response(JSON.stringify({ error: 'DUPLICATE_ENTRY', message: msg.split('DUPLICATE_ENTRY:')[1]?.trim() || msg }), { status: 409, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
         }
         if (msg.includes('SEQUENCE_ERROR:')) {
           return new Response(JSON.stringify({ error: 'SEQUENCE_ERROR', message: msg.split('SEQUENCE_ERROR:')[1]?.trim() || msg }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
         }
         if (msg.includes('SCHEDULE_ERROR:')) {
           return new Response(JSON.stringify({ error: 'SCHEDULE_ERROR', message: msg.split('SCHEDULE_ERROR:')[1]?.trim() || msg }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
         }
    ...
    if (req.method === 'PUT') {
      if (params.length !== 1) {
         return new Response(JSON.stringify({ error: 'BAD_REQUEST', message: 'ID do registro não fornecido' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      ...
      const { data, error } = await supabase.from('time_entries').update(parsed.data).eq('id', id).select('id, timestamp, notes, updated_at').maybeSingle();
    ...
    if (req.method === 'DELETE') {
      ...
      const { data: existing, error: existError } = await supabase.from('time_entries').select('id').eq('id', id).maybeSingle();
      if (existError) throw existError;
      if (!existing) {
         return new Response(JSON.stringify({ error: 'NOT_FOUND', message: 'Registro de ponto não encontrado' }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
    ...
    return new Response(JSON.stringify({ error: 'INTERNAL_ERROR', message: error.message || 'Erro interno no servidor' }), {
