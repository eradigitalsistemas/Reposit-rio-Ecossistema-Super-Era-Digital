...
serve(async (req) => {
  ...
  console.log(
    JSON.stringify({ stage: 'raw_body_received', body_size: JSON.stringify(body).length }),
  )
  ...
  try {
    ...
    console.log(
      JSON.stringify({ stage: 'instance_validation', success: true, instanceId: instance.id }),
    )
