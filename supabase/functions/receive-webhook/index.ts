...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: 'event_insert_failed', detail: String(e) }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    })
  }
  ...
  if (eventType !== 'messages') {
    return new Response(JSON.stringify({ ok: true, event_id: eventId, processed: 'event_only' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    })
  }
  ...
  try {
    ...
    if (!remoteJid) {
      return new Response(
        JSON.stringify({ ok: true, event_id: eventId, processed: 'no_remote_jid' }),
    ...
    const tsIso = new Date(
      typeof tsMs === 'number' ? (tsMs > 1e12 ? tsMs : tsMs * 1000) : Date.now(),
    ).toISOString()
    ...
    const messageRow = {
      ...
      transcription_status:
        mediaUrl && (mappedType === 'audio' || mappedType === 'image') && !contact!.is_group
          ? 'pending'
          : null,
    }
    ...
    const { data: inserted, error: msgErr } = await supabase
      ...
      .upsert(messageRow, { onConflict: 'uazapi_message_id', ignoreDuplicates: false })
    ...
    return new Response(
      JSON.stringify({
        ...
        queued_media: inserted?.transcription_status === 'pending',
      }),
      { status: 200, headers: { 'Content-Type': 'application/json' } },
    )
  } catch (e) {
    return new Response(
      JSON.stringify({ ok: true, event_id: eventId, normalize_error: String(e) }),
      { status: 200, headers: { 'Content-Type': 'application/json' } },
    )
  }
})
