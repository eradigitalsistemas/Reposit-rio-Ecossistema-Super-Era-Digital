...
function safeErr(e) {
  ...
  if (e instanceof Error) return { name: e.name, message: e.message, stack: String(e.stack || "").slice(0, 500) };
  if (typeof e === "object") {
    const o = e;
    return {
...
function extractText(msg) {
  ...
  if (msg.content) {
    ...
    if (typeof msg.content.conversation === "string" && msg.content.conversation) return msg.content.conversation;
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ctRes = await pgUpsert("whatsapp_contacts", {
    ...
    if (!uazapiMsgId) {
      return jsonResp(200, { ok: true, event_id: eventId, contact_id: contactId, warn: "no_message_id" });
    }

    const mediaUrl = (message.content && (message.content.URL || message.content.url || message.content.directPath)) || null;
    const mediaMime = (message.content && message.content.mimetype) || null;
    const tsIso = parseTimestamp(message.messageTimestamp, nowIso);
    const transcriptionStatus = (mappedType === "audio" || mappedType === "image") && !isGroup && mediaUrl ? "pending" : null;
    ...
    if (msgRes.error) console.error("message_upsert_error", msgRes.error, "row_summary", { id: uazapiMsgId, type: mappedType, ts: tsIso });
