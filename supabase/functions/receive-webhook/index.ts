...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (typeof event !== 'string' || typeof instanceName !== 'string') {
      ...
      return new Response(
        JSON.stringify({ error: 'Missing required fields: event and instance must be strings' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        },
    ...
    console.log(
      JSON.stringify({ event, instance: instanceName, timestamp: new Date().toISOString() }),
    )
    ...
    if (insertEvtErr) {
      console.error(
        JSON.stringify({
          error: insertEvtErr.message,
          context: 'insert whatsapp_events',
          payload_summary: { event, instanceName },
        }),
      )
    } else {
    ...
    if (!data) {
      return new Response(
        JSON.stringify({ success: true, ignored: true, reason: 'No data payload' }),
        {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        },
    ...
    if (event === 'messages') {
        ...
        try {
          ...
          if (existingContact) {
            ...
            if (newContact) {
              ...
              console.log(
                JSON.stringify({ action: 'insert', table: 'whatsapp_contacts', status: 'success' }),
              )
            }
          }
          ...
          if (upsertErr) {
            ...
            console.log(
              JSON.stringify({ action: 'upsert', table: 'whatsapp_messages', status: 'success' }),
            )
          }
      ...
      for (const update of updates) {
        try {
          ...
          if (updErr) {
            ...
            console.log(
              JSON.stringify({ action: 'update', table: 'whatsapp_messages', status: 'success' }),
            )
          }
        } catch (updErr: any) {
          console.error(
      ...
      try {
        ...
        if (remoteJid) {
          ...
          if (presErr) {
            ...
            console.log(
              JSON.stringify({ action: 'update', table: 'whatsapp_contacts', status: 'success' }),
            )
          }
        }
      } catch (presErr: any) {
      ...
      try {
        ...
        if (connErr) {
          ...
          console.log(
            JSON.stringify({ action: 'update', table: 'whatsapp_instances', status: 'success' }),
          )
        }
      } catch (connErr: any) {
