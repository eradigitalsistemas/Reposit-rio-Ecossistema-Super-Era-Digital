...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return createJsonResponse({ success: false, error: 'Authorization header required' }, 401, corsHeaders);
    }
    ...
    if (user && !authError) {
      ...
      try {
        ...
        return createJsonResponse({ success: false, error: 'Invalid token', details: 'JWT decode failed' }, 401, corsHeaders);
      }
    ...
    if (!instance_id || !phone || !message) {
      return createJsonResponse({ success: false, error: 'Missing required fields: instance_id, phone, message' }, 400, corsHeaders);
    }

    if (typeof message !== 'string' || message.length > 4096) {
      return createJsonResponse({ success: false, error: 'Message too long or invalid' }, 400, corsHeaders);
    }
    ...
    if (!userId) {
      ...
      return createJsonResponse({ success: false, error: 'Auth resolution failed' }, 500, corsHeaders);
    }

    console.log(JSON.stringify({ stage: 'auth_validated', user_id: userId, auth_path: authPath, token_role: tokenRole, version: VERSION }));
    ...
    if (instance.status !== 'connected') {
      return createJsonResponse({ success: false, error: `Instance not connected. Status: ${instance.status}` }, 400, corsHeaders);
    }
    ...
    if (existingContact) {
      ...
      if (insertContactError || !newContact) {
        return createJsonResponse({ success: false, error: 'Failed to create contact', details: insertContactError?.message }, 500, corsHeaders);
      }
    ...
    if (insertError || !inserted) {
      return createJsonResponse({ success: false, error: 'Failed to insert message', details: insertError?.message }, 500, corsHeaders);
    }
    ...
    if (!baseUrl) {
      return createJsonResponse({ success: false, error: 'No base URL configured' }, 500, corsHeaders);
    }
    ...
    try {
      ...
      return createJsonResponse({ success: false, error: 'uazapi error', details: err.message || 'Network error' }, 502, corsHeaders);
    }
    ...
    try {
      ...
      return createJsonResponse({ success: false, error: 'uazapi error', details: `HTTP ${uazapiStatus}` }, 502, corsHeaders);
    }
    ...
    if (!ok) {
      ...
      return createJsonResponse({ success: false, error: 'uazapi error', details: errorMsg }, 502, corsHeaders);
    }
    ...
    return createJsonResponse(
      { success: true, message_id: inserted.id, uazapi_message_id: uazapiMessageId, status: 'sent' },
      200,
      corsHeaders
    );
  } catch (err: any) {
