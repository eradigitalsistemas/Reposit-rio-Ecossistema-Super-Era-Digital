  ...
  try {
    ...
    if (req.method === 'GET' && !id) {
      ...
      if (search && search.length < 2) {
        return new Response(JSON.stringify({ error: 'Busca muito ampla. Digite pelo menos 2 caracteres.' }), {
          status: 400,
      ...
      if (search) {
        query = query.or(`name.ilike.%${search}%,email.ilike.%${search}%,profession.ilike.%${search}%`)
      }

      query = query.order(sort_by === 'nome' ? 'name' : 'created_at', { ascending: sort_order === 'asc' })
    ...
    if (req.method === 'PUT' && id && !action) {
      ...
      const { data: existing, error: findError } = await supabase.from('candidates').select('id').eq('id', id).single()
      ...
      const { data, error } = await supabase.from('candidates').update(parsed.data).eq('id', id).select().single()
      if (error) throw error
    ...
    if (req.method === 'POST' && id && action === 'convert') {
      ...
      const { data: existingEmployee } = await supabase.from('employees').select('id').or(`cpf.eq.${parsed.data.cpf}${parsed.data.rg ? `,rg.eq.${parsed.data.rg}` : ''}`).maybeSingle()
      if (existingEmployee) {
        return new Response(JSON.stringify({ error: 'Conflito de dados: CPF ou RG já cadastrado' }), {
          status: 409,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        })
      }
      ...
      const { data: newEmployee, error: employeeError } = await supabase.from('employees').insert(employeeData).select().single()
      if (employeeError) {
