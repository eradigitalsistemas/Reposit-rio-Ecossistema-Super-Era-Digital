...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { lead_id, message, phone, user_id, uazapi_url, uazapi_token } = body
    const instanceName = body.instanceName || Deno.env.get('INSTANCE_NAME') || 'comercial_era'

    if (!lead_id || !message || !user_id || !phone) {
      throw new Error('Campos obrigatórios ausentes')
    }

    const UAZAPI_URL = uazapi_url || Deno.env.get('UAZAPI_URL')
    const UAZAPI_TOKEN = uazapi_token || Deno.env.get('UAZAPI_TOKEN')
