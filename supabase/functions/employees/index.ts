...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === 'PUT' && id) {
      ...
      const { data: oldData, error: oldError } = await supabase.from('employees').select('*').eq('id', id).single()
      if (oldError || !oldData) {
        return new Response(JSON.stringify({ error: 'Colaborador não encontrado' }), {
          status: 404,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      ...
      const { data, error } = await supabase.from('employees').update(parsed.data).eq('id', id).select().single()
    ...
    if (req.method === 'DELETE' && id) {
      ...
      const { data, error } = await supabase.from('employees').update({ status: 'Demitido' }).eq('id', id).select().single()
      if (error) throw error
