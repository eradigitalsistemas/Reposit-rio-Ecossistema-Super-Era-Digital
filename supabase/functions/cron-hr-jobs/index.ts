...
async function logExecution(jobName: string, status: string, recordsProcessed: number, errors: any[]) {
...
async function calculateVacations() {
  ...
  try {
        ...
        if (years > 0) {
          for (let y = 1; y <= years; y++) {
            ...
            if (!existing) {
              ...
              await withRetry(() => supabase.from('vacation_balance').insert({
                employee_id: emp.id,
                year: targetYear,
                days_accrued: 30,
                days_remaining: 30,
    ...
    await logExecution('calculate_vacations', errors.length > 0 ? 'partial_success' : 'success', processed, errors);
  } catch (err: any) {
...
async function notifyVacationExpiration() {
  ...
  try {
    ...
    const activeBalances = balances?.filter((b: any) => ['Ativo', 'Em Experiência'].includes(b.employees?.status)) || [];
      ...
      try {
        ...
        if (!existing) {
          ...
          if (bal.employees?.department_id) {
            const { data: dept } = await supabase.from('departments').select('manager_id').eq('id', bal.employees.department_id).single();
          ...
          for (const uid of notifyIds) {
            await withRetry(() => supabase.from('notificacoes').insert({
              usuario_id: uid,
    ...
    await logExecution('notify_vacation_expiration', errors.length > 0 ? 'partial_success' : 'success', processed, errors);
  } catch (err: any) {
...
async function notifyProbationEnding() {
  ...
  try {
    ...
    await logExecution('notify_probation_ending', errors.length > 0 ? 'partial_success' : 'success', processed, errors);
  } catch (err: any) {
  ...
  try {
    ...
    for (const emp of employees || []) {
      try {
        ...
        const hasMissing = docs.length === 0 || docs.some((d: any) => d.status === 'Pendente' || d.status === 'Vencido');
    ...
    await logExecution('notify_missing_docs', errors.length > 0 ? 'partial_success' : 'success', processed, errors);
  } catch (err: any) {
...
async function updateProbationStatus() {
  ...
  try {
    ...
    await logExecution('update_probation_status', errors.length > 0 ? 'partial_success' : 'success', processed, errors);
  } catch (err: any) {
