...
async function getDecryptedUrl(uazapiMessageId: string): Promise<{ url: string; mime: string }> {
  ...
  if (!res.ok)
    throw new Error(`UAZAPI download failed ${res.status}: ${(await res.text()).slice(0, 300)}`)
      ...
      messages: [
        {
          ...
          content: [
            {
              type: 'text',
              text: 'Descreva esta imagem de forma concisa em portugues do Brasil em ate 2 frases. Se houver texto visivel, transcreva-o ao final apos "Texto: ".',
            },
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (msg.transcription_status === 'completed') {
      return new Response(JSON.stringify({ skipped: 'already_completed' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      })
    }
    ...
    if (msg.contact_id) {
      ...
      if (contact?.is_group) {
        await supabase
          .from('whatsapp_messages')
          .update({
            transcription_status: 'skipped_group',
            transcription_completed_at: new Date().toISOString(),
          })
          .eq('id', messageId)
        return new Response(JSON.stringify({ skipped: 'group_chat' }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        })
      }
    ...
    await supabase
      .from('whatsapp_messages')
      .update({ transcription_status: 'skipped_unsupported_type' })
      .eq('id', messageId)
    return new Response(JSON.stringify({ skipped: 'unsupported_type', type: msg.type }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    })
  } catch (e) {
