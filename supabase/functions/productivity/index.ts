  ...
  try {
    ...
    if (type === 'team' && paramId) {
       ...
       if (!employees || employees.length === 0) {
          return new Response(JSON.stringify({ data: { closed_demands: 0, avg_resolution_time_hours: 0, completion_rate: 0, employees: [] } }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
       }
       ...
       if (!employee) {
          ...
          if (usuario) {
            ...
            employeesData = [usuario]
             ...
             return new Response(JSON.stringify({ error: 'Colaborador não possui acesso ao sistema de Kanban' }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
          }
       }
    } else {
    ...
    const employeeMetrics = employeesData.map(emp => {
      ...
      empDemands.forEach(d => {
        ...
        const closedInPeriod = isClosed && d.data_conclusao && new Date(d.data_conclusao) >= startDate;
        ...
        if (closedInPeriod) {
          ...
          if (d.data_conclusao && d.data_atribuicao) {
            const resolutionTimeMs = new Date(d.data_conclusao).getTime() - new Date(d.data_atribuicao).getTime();
      ...
      return {
        ...
        avg_resolution_time_hours: closedCount > 0 ? parseFloat(((totalResolutionTimeMs / closedCount) / (1000 * 60 * 60)).toFixed(2)) : 0,
        completion_rate: totalAssigned > 0 ? parseFloat(((closedCount / totalAssigned) * 100).toFixed(2)) : 0,
      }
    })
    ...
    const result = {
      ...
      avg_resolution_time_hours: globalClosedCount > 0 ? parseFloat(((globalTotalResolutionTimeMs / globalClosedCount) / (1000 * 60 * 60)).toFixed(2)) : 0,
      completion_rate: globalTotalAssigned > 0 ? parseFloat(((globalClosedCount / globalTotalAssigned) * 100).toFixed(2)) : 0,
