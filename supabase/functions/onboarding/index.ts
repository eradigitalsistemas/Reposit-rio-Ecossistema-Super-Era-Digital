...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!employee_id) {
        return new Response(JSON.stringify({ error: 'ID do colaborador não informado' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (req.method === 'POST' && !subAction) {
        ...
        const { data: existing, error: extErr } = await supabase.from('onboarding_checklist').select('id').eq('employee_id', employee_id).limit(1)
        if (existing && existing.length > 0) {
            return new Response(JSON.stringify({ error: 'Checklist já existe' }), { status: 409, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
        }
        ...
        const { data: inserted, error: insErr } = await supabase.from('onboarding_checklist').insert(tasks).select()
        ...
        if (!experienceEndDate && emp.hire_date) {
            ...
            await supabase.from('employees').update({ experience_end_date: experienceEndDate }).eq('id', employee_id)
        }

        return new Response(JSON.stringify({ data: inserted, experience_end_date: experienceEndDate, success: true }), { status: 201, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }

    // GET /onboarding/:employee_id
    if (req.method === 'GET' && !subAction) {
        const { data, error } = await supabase.from('onboarding_checklist').select('*').eq('employee_id', employee_id).order('created_at', { ascending: true })
    ...
    if (req.method === 'PUT' && subAction === 'tasks' && taskId) {
        ...
        const { data: emp, error: empErr } = await supabase.from('employees').select('hire_date').eq('id', employee_id).single()
        if (empErr || !emp) {
            return new Response(JSON.stringify({ error: 'Colaborador não encontrado' }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
        }
        ...
        if (completed && emp.hire_date) {
            ...
            if (completed_at < hireDate) {
                return new Response(JSON.stringify({ error: 'Data de conclusão não pode ser anterior a data de contratação' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
            }
        }

        const { data: task, error: taskErr } = await supabase.from('onboarding_checklist').select('*').eq('employee_id', employee_id).eq('id', taskId).single()
        if (taskErr || !task) {
        ...
        const { data: updated, error: updErr } = await supabase.from('onboarding_checklist').update({
            completed,
        ...
        return new Response(JSON.stringify({ data: updated, success: true }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (req.method === 'GET' && subAction === 'documents') {
        ...
        const { data: docs, error: docsErr } = await supabase.from('documents').select('*').eq('employee_id', employee_id)
    ...
    return new Response(JSON.stringify({ error: 'Método não permitido ou rota inválida' }), { status: 405, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
