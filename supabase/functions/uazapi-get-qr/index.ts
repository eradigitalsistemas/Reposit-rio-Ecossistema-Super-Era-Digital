...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const instanceName = body.instanceName || body.uazapi_instance || Deno.env.get('INSTANCE_NAME') || 'comercial_era'
    ...
    if (state === 'open') {
      return new Response(JSON.stringify({ state: 'open' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
