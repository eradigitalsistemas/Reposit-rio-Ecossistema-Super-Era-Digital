...
async function generateAdminNotifications(supabase: any, adminId: string) {
    ...
    if (entries) {
        ...
        for (const t of entries) {
            if (!delaysByEmp[t.employee_id]) {
                delaysByEmp[t.employee_id] = { count: 0, name: t.employees?.personal_data?.nome || 'Colaborador' }
            }
    ...
    if (newNotifs.length > 0) {
        await supabase.from('notificacoes').upsert(newNotifs, { onConflict: 'usuario_id, referencia_id', ignoreDuplicates: true })
    }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!paramId) {
        return new Response(JSON.stringify({ error: 'Parâmetro não informado' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }

    if (req.method === 'GET') {
         const { data: userProfile } = await supabase.from('usuarios').select('id, perfil').eq('id', paramId).maybeSingle()
    ...
    if (req.method === 'PUT') {
         const { data: existing, error: errExist } = await supabase.from('notificacoes').select('id').eq('id', paramId).maybeSingle()
