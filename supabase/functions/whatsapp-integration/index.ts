...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (provider === 'uazapi') {
      ...
      const evolution_api_url = whatsapp_config?.evolution_api_url || Deno.env.get('EVOLUTION_API_URL')
      ...
      if (evolution_api_url && evolution_api_key && evolution_instance && phone) {
        ...
        const evoUrl = evolution_api_url.endsWith('/') ? evolution_api_url.slice(0, -1) : evolution_api_url
