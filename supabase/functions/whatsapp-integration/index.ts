...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (provider === 'uazapi') {
      ...
      const evolution_api_url = whatsapp_config?.evolution_api_url
      const evolution_api_key = whatsapp_config?.evolution_api_key
      const evolution_instance = whatsapp_config?.evolution_instance
      ...
      if (evolution_api_url && evolution_api_key && evolution_instance) {
        ...
        //   body: JSON.stringify({ number: phone, options: { delay: 1200 }, textMessage: { text: message } })
        // })
