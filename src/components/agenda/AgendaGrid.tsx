...
const AgendaDay = memo(({ day, isCurrentMonth, isToday, isRightEdge, isBottomEdge, dayEvents, onDayClick, onEventClick }: any) => {
return (
  <div
...
return prev.isCurrentMonth === next.isCurrentMonth &&
       prev.isToday === next.isToday &&
       prev.dayEvents.length === next.dayEvents.length &&
       prev.dayEvents.every((e: any, i: number) => e.id === next.dayEvents[i].id && e.data_inicio === next.dayEvents[i].data_inicio)
})

export const AgendaGrid = memo(function AgendaGrid({ currentDate, eventos, onDayClick, onEventClick }: AgendaGridProps) {
const monthStart = startOfMonth(currentDate)  const monthEnd = endOfMonth(monthStart)
  ...
  const eventsByDay = useMemo(() => {
    ...
    days.forEach(day => {
      ...
      isSameDayEvents.sort((a, b) => new Date(a.data_inicio).getTime() - new Date(b.data_inicio).getTime())
