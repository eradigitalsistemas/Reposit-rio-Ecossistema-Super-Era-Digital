...
//   servicos: jsonb (nullable, default: '[]'::jsonb)
// Table: colaboradores
//   id: uuid (not null, default: gen_random_uuid())
//   nome: text (not null)
//   email: text (not null)
...
//   tipo_demanda: text (nullable, default: 'Geral'::text)
//   checklist_id: uuid (nullable)
//   usuario_id: uuid (nullable)
//   data_criacao: timestamp with time zone (nullable, default: now())
//   responsavel_id: uuid (nullable)
//   departamento: text (nullable, default: 'Geral'::text)
// Table: demandas
//   id: uuid (not null, default: gen_random_uuid())
//   cliente_id: uuid (nullable)
...
//   id: bigint (not null)
//   created_at: timestamp with time zone (not null, default: now())
// Table: protocolos_certificados
//   id: uuid (not null, default: gen_random_uuid())
//   numero: text (not null)
//   cliente: text (not null)
//   tipo: text (not null)
//   parceiro: text (not null, default: 'Novos Protocolos'::text)
//   data_criacao: timestamp with time zone (nullable, default: now())
//   usuario_id: uuid (nullable)
// Table: sync_logs
//   id: uuid (not null, default: gen_random_uuid())
...
//   transcription_completed_at: timestamp with time zone (nullable)
...
//   FOREIGN KEY agenda_eventos_lead_id_fkey: FOREIGN KEY (lead_id) REFERENCES leads(id) ON DELETE SET NULL
...
// Table: demand_templates
//   FOREIGN KEY demand_templates_checklist_id_fkey: FOREIGN KEY (checklist_id) REFERENCES checklist_templates(id) ON DELETE SET NULL
//   PRIMARY KEY demand_templates_pkey: PRIMARY KEY (id)
//   FOREIGN KEY demand_templates_responsavel_id_fkey: FOREIGN KEY (responsavel_id) REFERENCES usuarios(id) ON DELETE SET NULL
//   FOREIGN KEY demand_templates_usuario_id_fkey: FOREIGN KEY (usuario_id) REFERENCES auth.users(id) ON DELETE SET NULL
// Table: demandas
...
//   FOREIGN KEY historico_leads_usuario_id_fkey: FOREIGN KEY (usuario_id) REFERENCES auth.users(id) ON DELETE SET NULL
// Table: leads
...
// Table: parceiros_certificados
//   UNIQUE parceiros_certificados_nome_key: UNIQUE (nome)
//   PRIMARY KEY parceiros_certificados_pkey: PRIMARY KEY (id)
// Table: ponto_configuracoes
//   PRIMARY KEY ponto_configuracoes_pkey: PRIMARY KEY (id)
// Table: protocolos_certificados
//   PRIMARY KEY protocolos_certificados_pkey: PRIMARY KEY (id)
//   CHECK protocolos_certificados_tipo_check: CHECK ((tipo = ANY (ARRAY['PF'::text, 'PJ'::text, 'SafeID - 4 meses'::text, 'SafeID - 3 anos'::text])))
//   FOREIGN KEY protocolos_certificados_usuario_id_fkey: FOREIGN KEY (usuario_id) REFERENCES auth.users(id) ON DELETE SET NULL
// Table: sync_logs
...
// Table: agenda_eventos
//   Policy "allow_all_agenda_eventos" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true
// Table: auth_logs
...
//     WITH CHECK: true
// Table: demand_templates
//   Policy "allow_all_demand_templates" (ALL, PERMISSIVE) roles={public}
//     USING: true
// Table: demandas
...
//     USING: true
// Table: historico_leads
//   Policy "historico_leads_delete" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: true
...
//     USING: true
//   Policy "historico_leads_update" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: leads
...
//     USING: true
// Table: logs_auditoria
//   Policy "allow_all_logs_auditoria" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true
// Table: mapeamento_demandas_responsaveis
...
//     USING: true
// Table: parceiros_certificados
//   Policy "allow_all_parceiros_certificados" (ALL, PERMISSIVE) roles={public}
//     USING: true
//     WITH CHECK: true
// Table: ponto_configuracoes
...
//     USING: true
// Table: protocolos_certificados
//   Policy "allow_all_protocolos_certificados" (ALL, PERMISSIVE) roles={public}
//     USING: true
//     WITH CHECK: true
//   Policy "auth_all_protocolos" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true
// Table: sync_logs
...
// --- DATABASE FUNCTIONS ---
// FUNCTION create_demand_with_event(text, text, text, text, timestamp with time zone, uuid, uuid, uuid, text, jsonb, jsonb, boolean, text, text, timestamp with time zone, timestamp with time zone, text)
//   CREATE OR REPLACE FUNCTION public.create_demand_with_event(p_titulo text, p_descricao text, p_prioridade text, p_status text, p_data_vencimento timestamp with time zone, p_responsavel_id uuid, p_cliente_id uuid, p_usuario_id uuid, p_tipo_demanda text, p_anexos jsonb, p_checklist jsonb, p_create_event boolean, p_event_titulo text, p_event_descricao text, p_event_data_inicio timestamp with time zone, p_event_data_fim timestamp with time zone, p_event_tipo text)
//    RETURNS jsonb
//    LANGUAGE plpgsql
//    SECURITY DEFINER
//   AS $function$
...
//   $function$
//   
// FUNCTION fn_notify_demand_assigned()
...
//   trg_notify_demand_assigned: CREATE TRIGGER trg_notify_demand_assigned AFTER INSERT OR UPDATE ON public.demandas FOR EACH ROW EXECUTE FUNCTION fn_notify_demand_assigned()
...
// Table: leads
//   trg_log_lead_changes: CREATE TRIGGER trg_log_lead_changes AFTER UPDATE ON public.leads FOR EACH ROW EXECUTE FUNCTION fn_log_lead_changes()
// Table: logs_auditoria
//   trg_update_demanda_atualizacao: CREATE TRIGGER trg_update_demanda_atualizacao AFTER INSERT ON public.logs_auditoria FOR EACH ROW EXECUTE FUNCTION update_demanda_atualizacao_on_log()
// Table: users
...
//   CREATE INDEX idx_demandas_data_criacao ON public.demandas USING btree (data_criacao DESC)
//   CREATE UNIQUE INDEX idx_demandas_protocolo ON public.demandas USING btree (protocolo)
//   CREATE INDEX idx_demandas_responsavel ON public.demandas USING btree (responsavel_id)
//   CREATE INDEX idx_demandas_status ON public.demandas USING btree (status)
...
// Table: parceiros_certificados
//   CREATE UNIQUE INDEX parceiros_certificados_nome_key ON public.parceiros_certificados USING btree (nome)
// Table: user_integrations
